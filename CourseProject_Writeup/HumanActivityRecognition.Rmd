...
####Gradient Boosting Method algorithm
not used here, seems only working with numeric y.
```{r}
#call.packages("gbm")
#model_gbm <- gbm(classe ~., data=subTraining, distribution="multinomial",n.trees = 1000,interaction.depth=2)
#pred_gbm <- predict(model_gbm, subTesting, n.trees=1000, type="link")
#res_gbm <- confusionMatrix(round(pred_gbm,1),subTesting$classe)
#res_gbm
```
